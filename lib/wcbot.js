...
WcBot.prototype._replyWithHttpsConnection = function(originalMessage) {
    ...
    https.request(this.settings.httpsOptions, function(response) {
        ...
        response.on('end', function() {
            ...
            var response;
            response = self._parseFreeWcJSON(str);
            self._sendResponseMessageToUser(response, user);
            response = self._parseOccupiedWcJSON(str);
            self._sendResponseMessageToUser(response, user);
            response = self._parseUnknownWcJSON(str);
            self._sendResponseMessageToUser(response, user);
        });
    }).end();
};

WcBot.prototype._sendResponseMessageToUser = function(response, user) {
...
WcBot.prototype._parseFreeWcJSON = function(message) {
    var wc = JSON.parse(message);
    var message = wc.filter(function(item) {
        return item.occupied == false;
    }).map(function(item) {
        return item.name;
    }).join(", ");
    return message.length > 0 ? "Wolne: " + message : '';
};

WcBot.prototype._parseOccupiedWcJSON = function(message) {
    var wc = JSON.parse(message);
    var message = wc.filter(function(item) {
    ...
    return message.length > 0 ? "Zajęte: " + message : '';
};

WcBot.prototype._parseUnknownWcJSON = function(message) {
    var wc = JSON.parse(message);
    var message = wc.filter(function(item) {
        return item.occupied != true && item.occupied != false;
    }).map(function(item) {
        return item.name;
    }).join(", ");
    return message.length > 0 ? "Nieustalone: " + message : '';
};
