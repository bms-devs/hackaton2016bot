...
WcBot.prototype._replyWithHttpsConnection = function(originalMessage) {
    ...
    https.request(this.settings.httpsOptions, function(response) {
        ...
        response.on('end', function() {
            ...
            self.postMessageToUser(user.name, self._parseFreeWcJSON(str), {
                as_user: true
            });
            self.postMessageToUser(user.name, self._parseOccupiedWcJSON(str), {
                as_user: true
            });
            self.postMessageToUser(user.name, self._parseUnknownWcJSON(str), {
                as_user: true
            });
        });
    }).end();
...
WcBot.prototype._parseFreeWcJSON = function(message) {
    ...
    return "Wolne: " + wc.filter(function(item) {
...
WcBot.prototype._parseOccupiedWcJSON = function(message) {
    var wc = JSON.parse(message);
    return "Zajęte: " + wc.filter(function(item) {
...
WcBot.prototype._parseUnknownWcJSON = function(message) {
    var wc = JSON.parse(message);
    return "Nieustalone: " + wc.filter(function(item) {
